...
import type { DBClient } from './FuneyPG'
/**
 * Update monthly interest for an account.
 */
export async function updateInterest(client: DBClient, userIdOrView: string): Promise<void> {
  return client
    .query(
      `
    SELECT 
...
WHERE 
  ...
  OR a.view = ?
    ...
    .then(({ rows }) => {
      ...
      return addTransaction(client, userid, 'Interest payment', sum * interest, true, next).then(
        (result) => {
...
/**
 * Update weekly allowance for an account.
 */
export async function updateAllowance(client: DBClient, userIdOrView: string): Promise<void> {
  return client
    .query(
      `
