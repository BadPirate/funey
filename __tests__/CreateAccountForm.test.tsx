...
describe('CreateAccountForm', () => {
  ...
  it('submits the form, calls the API, and redirects on successful creation', async () => {
    fetchMock.mockResponseOnce(
      JSON.stringify({ message: 'Account created successfully!', key: '1234abcd' }),
      { status: 201 },
    )
    ...
    await waitFor(() => {
      ...
      expect(mockPush).toHaveBeenCalledWith('/manage/1234abcd')
    })
  })

  it('shows an error message if username already exists', async () => {
    // Mock an API response indicating the user already exists
    fetchMock.mockResponseOnce(JSON.stringify({ error: 'Username already exists' }), {
      status: 409,
    })
  ...
  it('shows an error message if username is too short', async () => {
    ...
    expect(
      await screen.findByText(/Username must be at least 4 characters long/i),
    ).toBeInTheDocument()
